...
rates - rather than one flat rate per category. Seat capacity per (course,
term) similarly comes from backend/capacity_model.py's most-recent-year
figure in demand_history.json, falling back to the static seat_capacity in
courses.json where there's no history. Pass demand_history={} to opt back
into the old flat per-category rates and static per-course capacity (e.g. for
isolating other variables in a scenario comparison).
...
from backend.capacity_model import all_capacities_by_term, scale_capacity_in_history
...
class CohortSimulator:
    ...
    def __init__(
        ...
        self.course_term_capacity = (
            all_capacities_by_term(courses, demand_history) if demand_history else None
        )
    ...
    def run(self) -> dict:
        ...
        for term_idx in range(1, self.max_terms + 1):
            ...
            for code, students in requests.items():
                if self.course_term_capacity is not None:
                    cap = self.course_term_capacity.get((code, term), self.courses[code]["seat_capacity"])
                else:
...
def sensitivity_analysis(
    ...
    # 4) seat capacity halved for every course - via demand_history, not the
    # static courses.json field, since real recorded capacity takes priority
    # (see backend/capacity_model.py). Also halves the static fallback for
    # any course with no demand_history on record, so nothing is missed.
    halved_courses = json.loads(json.dumps(courses))
    for code, rec in halved_courses.items():
        if rec["seat_capacity"] is not None:
            rec["seat_capacity"] = max(1, rec["seat_capacity"] // 2)
    halved_history = scale_capacity_in_history(demand_history, 0.5)
    perturbations.append(("seat_capacity_halved", baseline_config, halved_courses, halved_history))
