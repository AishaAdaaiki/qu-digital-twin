...
with tab_rates:
    ...
    for code, record in sorted(courses.items()):
        for term in record.get("offered", []):
            ...
            static_cap = record.get("seat_capacity")
            eff_cap = cm.effective_capacity(courses, demand_history, code, term)
            rows.append({
                "Course": code,
                "Name": record["name"],
                ...
                "courses.json capacity": static_cap if static_cap is not None else "unlimited",
                "Actual capacity used": eff_cap if eff_cap is not None else "unlimited",
            })
    df = pd.DataFrame(rows)
    st.dataframe(df, use_container_width=True, height=400)
    mismatches = df[df["courses.json capacity"] != df["Actual capacity used"]]
    if len(mismatches):
        st.caption(
            f"⚠️ {len(mismatches)} course/term row(s) above have real recorded capacity that "
            "differs from (or overrides \"unlimited\" in) the static courses.json field — "
            "the simulator always uses the **Actual capacity used** column. See "
            "backend/capacity_model.py."
        )
